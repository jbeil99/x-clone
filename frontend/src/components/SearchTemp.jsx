...
const Top = ({ top }) => (
    <div className="max-w-xl mx-auto">
        ...
        {top.map((post, i) => (
            <Tweet tweet={post} key={post.id + i} />
        ))}
    </div>
);
...
export default function SearchTab({ top, latest }) {
    const [activeTab, setActiveTab] = useState('Top');
    let content;
    if (activeTab === 'Top') content = <Top top={top} />;
    else if (activeTab === 'Latest') content = <Latest latest={latest} />;
