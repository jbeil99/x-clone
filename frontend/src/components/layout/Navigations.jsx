...
export default function Navigations() {
    ...
    return (
        <div className="flex flex-col w-[72px] sm:w-[275px] px-2 sm:px-4 py-4 border-r border-gray-800 items-center sm:items-stretch sticky top-0 h-screen">
            <div>
                ...
                <nav className="space-y-1 flex flex-col items-center sm:items-stretch">
                    {sidebarItems.map((item) => (
                        <Link
                            key={item.text}
                            to={item.path}
                            className={`flex items-center gap-4 text-lg font-medium hover:bg-gray-900 px-3 py-3 rounded-full transition-colors relative ${location.pathname === item.path ? 'font-bold' : ''}`}
                        >
                            <div className="relative">
                                <item.icon className={`h-6 w-6 mx-auto ${location.pathname === item.path ? 'text-blue-500' : 'text-white'}`} />
                                {item.text === 'Messages' && unreadMessageCount > 0 && (
                                    <div className="absolute -top-1 -right-1 bg-blue-500 text-white text-xs font-bold rounded-full min-w-[16px] h-[16px] flex items-center justify-center px-1">
